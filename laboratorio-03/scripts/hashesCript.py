...
def main():
    parser = argparse.ArgumentParser(
        description="Gera hashes a partir de uma entrada de texto ou arquivo e opcionalmente descriptografa"
    )
    parser.add_argument("--texto", help="Texto a ser hasheado ou criptografado")
    parser.add_argument(
        "--arquivo", help="Caminho para um arquivo a ser hasheado ou criptografado"
    )
    parser.add_argument(
        "--tipo",
        choices=["md5", "sha1", "sha256", "sha512", "base64"],
        help="Tipo de hash ou criptografia",
    )
    parser.add_argument(
        "--descriptografar",
        action="store_true",
        help="Descriptografar o texto ou arquivo",
    )
